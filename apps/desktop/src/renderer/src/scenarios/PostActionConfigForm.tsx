    ...
    <div className="post-action-flow">
          ...
          <div className={toggleClass(postToWall)}>
            <label className="post-target-toggle">
              <input
                type="checkbox"
                checked={postToWall}
                onChange={(event) => onChange('postToWall', event.target.checked)}
              />
              <span>
                <strong>Đăng tường Page</strong>
                <small>Đăng trực tiếp lên tường Page.</small>
              </span>
            </label>
            {postToWall ? (
              <div className="post-target-fields">
                <label className="scenario-field">
                  ...
                  <input
                    aria-label="Page UID"
                    value={stringValue(config, 'wallPageUid')}
                    placeholder="Nhập Page UID..."
                    maxLength={200}
                    onChange={(event) => onChange('wallPageUid', event.target.value)}
                  />
                </label>
                <label className="scenario-field compact-number">
          ...
          <div className={toggleClass(postToGroups)}>
            <label className="post-target-toggle">
              <input
                ...
                onChange={(event) => onChange('postToGroups', event.target.checked)}
              />
              <span>
                <strong>Đăng nhóm</strong>
                <small>Mỗi dòng một Group UID hoặc URL Facebook.</small>
              </span>
            </label>
            {postToGroups ? (
      ...
      <section className="post-config-block">
        ...
        <div className="post-content-layout">
          ...
          <div className="post-selection-panel">
            ...
            <div className="post-segmented" role="radiogroup" aria-label="Cách lấy bài">
              <label className={selectionMode === 'sequential' ? 'selected' : ''}>
                <input
                  type="radio"
                  name="post-selection-mode"
                  value="sequential"
      ...
      <section className="post-config-block compact">
        <div className="post-config-heading">
          ...
          <div>
            <strong>Delay giữa mỗi bài</strong>
            <small>Khoảng nghỉ áp dụng cho các lượt đăng của action này.</small>
          </div>
        </div>
        ...
        <div className="post-delay-row">
          <label>
            ...
            <input
              ...
              min={0}
              max={86_400}
              value={numberValue(config, 'postDelayMinSeconds', 200)}
              onChange={(event) => onChange('postDelayMinSeconds', event.target.value === '' ? undefined : Number(event.target.value))}
            />
          </label>
          ...
          <label>
            <span>Đến</span>
            <input
              aria-label="Delay đến"
              type="number"
              min={0}
              max={86_400}
              value={numberValue(config, 'postDelayMaxSeconds', 300)}
              onChange={(event) => onChange('postDelayMaxSeconds', event.target.value === '' ? undefined : Number(event.target.value))}
            />
          </label>
