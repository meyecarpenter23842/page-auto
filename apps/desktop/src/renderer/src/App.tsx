...
type RouteId = 'overview' | 'accounts' | 'hotmail' | 'page-tabs' | 'scenarios' | 'logs' | 'settings'
...
const routeDescriptions: Record<RouteId, { title: string; description: string }> = {
  ...
  scenarios: { title: 'Kịch Bản', description: 'Quản lý các kịch bản và action module tái sử dụng. K1 chưa gắn kịch bản vào Page hoặc Tài khoản.' },
  ...
  return (
    <div className="app-shell">
      ...
      <main key={activeRoute} className={`${workspaceClass} workspace-transition`}>
        <header className="topbar"><div><p className="eyebrow">PAGE-AUTO / {activeRoute === 'hotmail' ? 'EMAIL' : activeRoute === 'scenarios' ? 'KỊCH BẢN' : activeRoute.toUpperCase()}</p><h1>{active.title}</h1></div><div className="version-badge">{appInfo ? `v${appInfo.version}` : 'Loading...'}</div></header>

        {activeRoute === 'accounts' ? <AccountManager /> : activeRoute === 'hotmail' ? <HotmailAuto /> : activeRoute === 'page-tabs' ? <PageBusinessWorkspace /> : activeRoute === 'scenarios' ? <ScenarioManager /> : activeRoute === 'logs' ? <ExecutionLogs /> : activeRoute === 'settings' ? <SettingsPanel appInfo={appInfo} /> : (
          <>
