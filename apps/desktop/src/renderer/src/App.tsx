...
const routes: Route[] = [
  ...
  { id: 'proxy-builder', label: 'Proxy Center' },
  { id: 'zalo', label: 'Zalo' },
...
const routeDescriptions: Record<RouteId, { title: string; description: string }> = {
  ...
  'proxy-builder': { title: 'Proxy Center', description: 'Tạo, lưu kho, kiểm tra và gán proxy cho Account từ một workspace; provision và network runtime vẫn nằm ngoài Renderer.' },
    ...
    <div className="app-shell">
      ...
      <main key={activeRoute} className={`${workspaceClass} workspace-transition`}>
        <header className="topbar">
          <div><p className="eyebrow">PAGE-AUTO / {activeRoute === 'hotmail' ? 'EMAIL' : activeRoute === 'actions' ? 'HÀNH ĐỘNG' : activeRoute === 'scanner' ? 'QUÉT DỮ LIỆU' : activeRoute === 'proxy-builder' ? 'PROXY CENTER' : activeRoute === 'content-library' ? 'THƯ VIỆN' : activeRoute === 'zalo' ? 'ZALO' : activeRoute.toUpperCase()}</p><h1>{active.title}</h1></div>
