...
type RouteId = 'accounts' | 'hotmail' | 'content-library' | 'page-tabs' | 'scenarios' | 'logs' | 'settings'
  ...
  return (
    <div className="app-shell">
      ...
      <main key={activeRoute} className={`${workspaceClass} workspace-transition`}>
        ...
        {activeRoute === 'accounts' ? <AccountManager /> : activeRoute === 'hotmail' ? <HotmailAuto /> : activeRoute === 'content-library' ? <ContentLibraryHub /> : activeRoute === 'page-tabs' ? <PageBusinessWorkspace /> : activeRoute === 'scenarios' ? <ScenarioWorkspace /> : activeRoute === 'logs' ? <ExecutionLogs /> : <SettingsPanel appInfo={appInfo} />}
      </main>
    </div>
  )
}
