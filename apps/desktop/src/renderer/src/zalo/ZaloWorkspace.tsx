...
type ZaloSubTab = 'accounts' | 'automation'
...
export function ZaloWorkspace() {
  const [activeTab, setActiveTab] = useState<ZaloSubTab>('automation')
  ...
  return (
    <section className="panel-stack zalo-tool-shell">
      <div className="zalo-workspace-titlebar">
        <div><span>Zalo</span><strong>{activeTab === 'accounts' ? 'Tài khoản Zalo' : 'Gửi tin / Kết bạn'}</strong></div>
      ...
      <nav className="zalo-subtabs" aria-label="Zalo workspace">
        <button type="button" className={activeTab === 'accounts' ? 'active' : ''} onClick={() => setActiveTab('accounts')}>Tài khoản Zalo</button>
        <button type="button" className={activeTab === 'automation' ? 'active' : ''} onClick={() => setActiveTab('automation')}>Gửi tin / Kết bạn</button>
      </nav>
