...
describe('page runtime presentation', () => {
  it('maps account and page runtime states to operator labels', () => {
    ...
    expect(accountRuntimeLabel('error')).toBe('Lỗi')
    expect(accountRuntimeLabel('error', '282')).toBe('Checkpoint 282')
    expect(accountRuntimeLabel('error', '956')).toBe('Checkpoint 956')
    expect(accountRuntimeLabel('error', 'unknown')).toBe('Checkpoint không xác định')
