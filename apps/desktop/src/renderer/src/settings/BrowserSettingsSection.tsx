...
type BusyState = 'save' | 'detect' | 'pick' | 'profileRoot' | 'test' | 'retile' | null
...
export function BrowserSettingsSection({ appInfo: _appInfo }: BrowserSettingsSectionProps) {
  ...
  const pickFacebookProfileRoot = async () => {
    setBusy('profileRoot'); setFeedback(null)
    try {
  ...
  return <div className="settings-section settings-section-with-actions">
    <div className="settings-section-content"><div className="browser-section">
      ...
      <div className="browser-form-grid">
        ...
        <label className="number-field"><span>Delay thao tác min</span><div><input type="number" min="0" max="30" step="0.5" value={(draft.actionDelayMinMs ?? DEFAULT_APP_SETTINGS.browser.actionDelayMinMs ?? 1000) / 1000} onChange={(event) => update('actionDelayMinMs', Math.max(0, Math.round(Number(event.target.value) * 1000)))} /><em>giây</em></div></label>
        <label className="number-field"><span>Delay thao tác max</span><div><input type="number" min="0" max="30" step="0.5" value={(draft.actionDelayMaxMs ?? DEFAULT_APP_SETTINGS.browser.actionDelayMaxMs ?? 3000) / 1000} onChange={(event) => update('actionDelayMaxMs', Math.max(0, Math.round(Number(event.target.value) * 1000)))} /><em>giây</em></div></label>
