...
export function BrowserSettingsSection({ appInfo }: BrowserSettingsSectionProps) {
  ...
  const grid = useMemo(() => layout ? browserTileGrid(layout) : { columns: 1, rows: 1, capacity: 1 }, [layout])
  ...
  const updateGrid = (columns: number, rows: number) => {
    setLayout((current) => current ? withBrowserGridDimensions(current, columns, rows) : current)
    setFeedback(null)
  }
  ...
  return <div className="settings-section settings-section-with-actions">
    <div className="settings-section-content"><div className="browser-section">
      ...
      <div className="browser-form-grid">
        ...
        <label className="toggle-card span-3"><div><strong>Compact / xếp nhiều Chrome</strong><small>Chia theo lưới 2 chiều. Cửa sổ và toàn bộ Facebook bên trong cùng co theo cả chiều ngang lẫn chiều dọc.</small></div><input type="checkbox" checked={layout.enabled} onChange={(event) => { updateLayout('enabled', event.target.checked); if (event.target.checked) update('mode', 'visible') }} /></label>
        {layout.enabled && <>
          <label className="number-field"><span>Cột ngang</span><div><input type="number" min="1" max="16" value={grid.columns} onChange={(event) => updateGrid(Math.max(1, Math.min(16, Number(event.target.value) || 1)), grid.rows)} /><em>cột</em></div></label>
          <label className="number-field"><span>Hàng dọc</span><div><input type="number" min="1" max="32" value={grid.rows} onChange={(event) => updateGrid(grid.columns, Math.max(1, Math.min(32, Number(event.target.value) || 1)))} /><em>hàng</em></div></label>
          <div className="field"><span>Tổng sức chứa</span><div className="test-result ok">{grid.capacity} Chrome · scale 2 chiều</div></div>
