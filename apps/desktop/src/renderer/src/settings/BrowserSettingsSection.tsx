  ...
  return <div className="settings-section settings-section-with-actions">
    <div className="settings-section-content"><div className="browser-section">
      ...
      <div className="browser-form-grid">
        ...
        <label className="toggle-card span-3"><div><strong>Compact / xếp nhiều Chrome</strong><small>Chọn N hàng dọc thì app xếp đúng N cột × N hàng. Facebook bên trong vẫn co để phủ kín từng ô.</small></div><input type="checkbox" checked={layout.enabled} onChange={(event) => { updateLayout('enabled', event.target.checked); if (event.target.checked) update('mode', 'visible') }} /></label>
        {layout.enabled && <>
          <label className="number-field"><span>Số hàng dọc</span><div><input type="number" min="1" max={MAX_COMPACT_GRID_SIDE} value={layout.rowCount} onChange={(event) => updateRows(Math.max(1, Math.min(MAX_COMPACT_GRID_SIDE, Number(event.target.value) || 1)))} /><em>hàng</em></div></label>
    ...
    <div className="inline-settings-actions"><span className={`inline-settings-feedback ${feedback?.kind ?? ''}`}>{feedback?.text ?? (dirty ? 'Có thay đổi chưa lưu.' : (grid ? `Chọn ${layout.rowCount} hàng dọc = ${grid.columns} × ${grid.rows}, sức chứa ${grid.capacity} Chrome.` : 'Compact xếp theo lưới vuông.'))}</span><div><button type="button" className="settings-button" disabled={busy !== null} onClick={() => { setDraft(copyBrowser(DEFAULT_APP_SETTINGS.browser)); setLayout(copyLayout(DEFAULT_BROWSER_WINDOW_LAYOUT)) }}>Mặc định</button><button type="button" className="settings-button" disabled={!dirty || busy !== null} onClick={() => { if (saved) setDraft(copyBrowser(saved)); if (savedLayout) setLayout(copyLayout(savedLayout)) }}>Hủy</button><button type="button" className="settings-button primary" disabled={!dirty || busy !== null} onClick={() => void save()}>{busy === 'save' ? 'Đang lưu...' : 'Lưu cài đặt'}</button></div></div>
  </div>
}
