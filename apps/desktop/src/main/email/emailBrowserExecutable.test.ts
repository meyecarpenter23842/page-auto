...
describe('email browser executable candidates', () => {
  it('keeps a manually selected executable authoritative', () => {
    expect(emailBrowserExecutableCandidates('', 'C:\\Custom\\chrome.exe', {}))
  ...
  it('looks beside the Email profile root before system installs when auto-detecting', () => {
    ...
    const candidates = emailBrowserExecutableCandidates(profileRoot, '', {})
  ...
  it('uses Email-local/system candidates only when Browser Auto is enabled', () => {
    const env = {
      PROGRAMFILES: 'C:\\Program Files',
      'PROGRAMFILES(X86)': 'C:\\Program Files (x86)',
      LOCALAPPDATA: 'C:\\Users\\PC\\AppData\\Local'
    }
    const candidates = emailBrowserExecutableCandidates('', '', env)
    ...
    expect(candidates).not.toContain('D:\\FacebookBrowser\\chrome.exe')
  })
})
