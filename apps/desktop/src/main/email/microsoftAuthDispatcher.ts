 ...
 * `retryable` both return control to the detector. `authenticated` is accepted
 * only when the detector itself proved the authenticated surface; a credential
 * handler cannot declare success immediately after clicking Submit. Unknown
...
export async function runMicrosoftAuthDispatchLoop<TDetection = unknown>(
  ...
  for (let step = 0; step < maxSteps; step += 1) {
    ...
    if (result.kind === 'needs_attention') return result
    if (result.kind === 'authenticated' && detected.surface === 'authenticated') return result
    // `handled`, `retryable`, and premature `authenticated` outcomes all require
    // a fresh detector pass before the controller can accept authentication.
  }
