...
export class PageWallRunNowService {
  ...
  async execute(payload: PageWallRunNowPayload): Promise<PageWallRunNowResult> {
    if (!Number.isInteger(payload.pageTabId) || payload.pageTabId <= 0) {
      return failure(payload, 'Page Tab không hợp lệ.')
    }
    if (!Number.isInteger(payload.accountId) || payload.accountId <= 0) {
      return failure(payload, 'Tài khoản chạy Đăng Tường không hợp lệ.', 'no_enabled_account')
    }

    const pageTab = this.pageTabs.get(payload.pageTabId)
    if (!pageTab) return failure(payload, 'Page Tab không còn tồn tại.')

    const accountRef = pageTab.accounts.find((account) => account.accountId === payload.accountId)
    if (!accountRef || !accountRef.enabled) {
      return failure(payload, 'Tài khoản không thuộc danh sách đang bật của Page Tab.', 'no_enabled_account')
    }

    const pageUid = pageTab.pageUid.trim()
    if (!pageUid) return failure(payload, 'Page Tab chưa có Page UID hợp lệ.', 'page_navigation_failed')
    ...
    if (unsupported) {
      return failure(payload, 'Danh sách ảnh có file không được hỗ trợ. Chỉ dùng JPG, JPEG, PNG hoặc WEBP.', 'media_failed')
    }

    if (!payload.content.trim() && imagePaths.length === 0) {
      return failure(payload, 'Hãy nhập nội dung hoặc chọn ít nhất một ảnh trước khi Đăng ngay.', 'no_content')
    }
