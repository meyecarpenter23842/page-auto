...
export class GroupWorkspaceRunnerService {
  ...
  start(workspaceId: number): InteractionWorkspaceRunSnapshot {
    ...
    this.log(active, 'info', `Bắt đầu phiên ${runId}: ${accounts.length} account, chạy lần lượt, action Tham gia nhóm.`)
  ...
  resume(workspaceId: number): InteractionWorkspaceRunSnapshot | null {
    ...
    active.snapshot.message = 'Đã tiếp tục snapshot Nhóm hiện tại.'
    ...
    this.log(active, 'info', 'Phiên Nhóm đã Resume; không đọc lại config đang sửa.')
    return cloneSnapshot(active.snapshot)
  }
