...
export class ScenarioGroupPostAdapter {
  ...
  async run(job: ScenarioActionWorkerJob, onLog?: (event: ActionLogEvent) => void): Promise<ScenarioActionWorkerResult> {
    ...
    if (!postingRunId) {
      ...
      this.emit(job, onLog, 'executing', 'info', `Đã tạo snapshot Group #${postingRunId}: ${created.metrics.total} Group cho action.`)
    }

    const postsPerAccount = Math.max(1, Math.floor(numberConfig(config, 'postsPerAccount', 1)))
    ...
    for (let index = 0; index < postsPerAccount; index += 1) {
      if (scope.cancelledRunKeys.has(job.request.runKey)) {
        return workerResult(config, { status: 'stopped', code: 'action_stopped', message: 'Đăng bài nhóm đã dừng theo yêu cầu.' }, startedAt, attempts)
      }
      ...
      if (mapped.status === 'success') success += 1
        ...
        return workerResult(config, mapped, startedAt, attempts, attentionSessionState(outcome))
      }
      ...
      if (index + 1 < postsPerAccount && !scope.cancelledRunKeys.has(job.request.runKey)) {
        ...
        if (delayMs > 0 && !await this.sleep(scope, job.request.runKey, delayMs)) {
          return workerResult(config, { status: 'stopped', code: 'action_stopped', message: 'Đăng bài nhóm đã dừng trong lúc chờ bài kế tiếp.' }, startedAt, attempts)
        }
      }
    }
