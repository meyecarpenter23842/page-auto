...
import type { Keyboard, Locator, Mouse, Page } from 'playwright-core'
...
describe('Facebook common interaction pacing', () => {
  ...
  it('bypasses pacing only inside time-critical authentication scopes', async () => {
    const fake = fakeFacebookPage()
    const page = createPacedFacebookPage(fake.page, fixedBrowserDelay)
    ...
    expect(fake.events).toEqual(['fill', 'click'])

    await page.locator('button').click()
    expect(fake.waits).toEqual([2500])
    expect(fake.events).toEqual(['fill', 'click', 'click'])
  })
