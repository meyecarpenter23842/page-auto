...
export class PageIdentitySwitcher {
  ...
  async switchTo(pageUid: string): Promise<PostingJobResult> {
    ...
    if ((await activeFacebookProfileId(this.context)) === pageUid) {
      return { status: 'success', message: 'Page identity đã active.' }
    }
    ...
    if (!switchControl) {
      return failure('page_identity_unconfirmed', 'Không tìm thấy control chuyển sang Page và không xác minh được Page identity hiện tại.')
    }

    try {
      await switchControl.click({ timeout: 15_000 })
    ...
    if ((await activeFacebookProfileId(this.context)) !== pageUid) {
      return failure('page_identity_unconfirmed', 'Đã bấm switch nhưng chưa xác minh được Page identity bằng session hiện tại.')
    }
...
export class GroupNavigator {
  ...
  async open(groupUid: string): Promise<PostingJobResult> {
    ...
    if (await unavailable.isVisible().catch(() => false)) {
      return failure('group_unavailable', 'Group không khả dụng với session hiện tại.')
    }
  ...
  async detect(container: Locator, content: string): Promise<PostingJobResult> {
    ...
    while (Date.now() < deadline) {
      ...
      if (await successToast.isVisible().catch(() => false)) {
        return { status: 'success', message: 'Facebook hiển thị trạng thái publish thành công.' }
      }
      ...
      if (composerClosed && fingerprint.length >= 12) {
        ...
        if (await publishedText.isVisible().catch(() => false)) {
          ...
          return {
            status: 'success',
            message: 'Đã xác minh nội dung xuất hiện trong Group sau publish.',
            ...(href ? { publishedUrl: href.startsWith('http') ? href : `https://www.facebook.com${href}` } : {})
          }
        }
      }
...
export async function executePostingJob(job: PostingJobRequest): Promise<PostingJobResult> {
  ...
  if (job.proxy) {
    launchOptions.proxy = {
      ...
      ...(job.proxy.username ? { username: job.proxy.username } : {}),
      ...(job.proxy.password ? { password: job.proxy.password } : {})
    }
  }
