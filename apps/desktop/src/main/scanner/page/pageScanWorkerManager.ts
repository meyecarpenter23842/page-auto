...
function isWorkerEvent(event: unknown): event is PageScanWorkerEvent {
...
class AsyncRecordQueue implements AsyncIterator<PageScanRawRecord>, AsyncIterable<PageScanRawRecord> {
  private readonly values: PageScanRawRecord[] = []
  private readonly waiters: Array<{
    resolve: (result: IteratorResult<PageScanRawRecord>) => void
    reject: (error: Error) => void
  }> = []
  private ended = false
  private failure: Error | null = null

  push(value: PageScanRawRecord): void {
  ...
  next(): Promise<IteratorResult<PageScanRawRecord>> {
  ...
  [Symbol.asyncIterator](): AsyncIterator<PageScanRawRecord> {
    return this
  }
}

export class PageScanWorkerManager {
  private readonly active = new Set<UtilityProcess>()

  constructor(
    private readonly getRuntimeSettings: () => RuntimeSettings,
    private readonly onSessionUpdate?: (accountId: number, update: PageScanWorkerSessionUpdate) => void
  ) {
    configureGlobalBrowserLaunchBroker(this.getRuntimeSettings)
  }

  async *run(job: PageScanWorkerJob, control: ScanAdapterControl): AsyncIterable<PageScanRawRecord> {
    ...
    const worker = utilityProcess.fork(join(__dirname, 'scanner-page-worker.js'), args, {
      serviceName: `PAGE-AUTO scanner page account ${job.accountId}`
    })
    ...
    worker.once('exit', (code) => {
      ...
      if (!terminal) {
        ...
        else queue.fail(new ScanAdapterRuntimeError('failed', `Page scanner worker đã thoát ngoài dự kiến (code ${code}).`))
      }
    })
