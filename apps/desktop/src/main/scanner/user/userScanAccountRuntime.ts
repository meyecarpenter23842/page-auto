...
import type { UserScanRawRecord, UserScanRuntime } from './userScanAdapter'
import { UserScanWorkerManager } from './userScanWorkerManager'
import type { UserScanWorkerJob, UserScanWorkerSessionUpdate } from './userScanWorkerContracts'

function runtimeAccount(account: AccountRecord): UserScanWorkerJob['sessionAccount'] {
  return {
    id: account.id,
    uid: account.uid,
...
export class UserScanAccountRuntime implements UserScanRuntime {
  private readonly accounts: AccountRepository
  private readonly settings: AppSettingsRepository
  private readonly accountExecution = new AccountExecutionCoordinator()
  private readonly workers: UserScanWorkerManager

  constructor(
    database: Database.Database,
    private readonly dataDirectory: string,
    workers?: UserScanWorkerManager
  ) {
    this.accounts = new AccountRepository(database)
    this.settings = new AppSettingsRepository(database)
    this.workers = workers ?? new UserScanWorkerManager(
      () => this.settings.get().runtime,
      (accountId, update) => this.persistSessionUpdate(accountId, update)
    )
  }

  async *scan(input: StartScanJobInput, control: ScanAdapterControl): AsyncIterable<UserScanRawRecord> {
    if (input.source.type !== 'account' || input.source.accountId === null) {
      throw new ScanAdapterRuntimeError('needs_attention', 'Quét Người dùng production cần nguồn Account Page-Auto.')
    }
    ...
    try {
      ...
      const job: UserScanWorkerJob = {
        runKey: `scanner-user-${accountId}-${Date.now()}-${Math.floor(Math.random() * 10000)}`,
        accountId,
        profileDirectory,
  ...
  private persistSessionUpdate(accountId: number, update: UserScanWorkerSessionUpdate): void {
    const account = this.accounts.getById(accountId)
    if (!account) return
    const now = Date.now()
