...
function isWorkerEvent(event: unknown): event is GroupScanWorkerEvent {
...
class AsyncRecordQueue implements AsyncIterator<GroupScanRawRecord>, AsyncIterable<GroupScanRawRecord> {
  private readonly values: GroupScanRawRecord[] = []
  private readonly waiters: Array<{
    resolve: (result: IteratorResult<GroupScanRawRecord>) => void
    reject: (error: Error) => void
  }> = []
  private ended = false
  private failure: Error | null = null

  push(value: GroupScanRawRecord): void {
  ...
  next(): Promise<IteratorResult<GroupScanRawRecord>> {
  ...
  [Symbol.asyncIterator](): AsyncIterator<GroupScanRawRecord> {
    return this
  }
}

export class GroupScanWorkerManager {
  private readonly active = new Set<UtilityProcess>()

  constructor(
    private readonly getRuntimeSettings: () => RuntimeSettings,
    private readonly onSessionUpdate?: (accountId: number, update: GroupScanWorkerSessionUpdate) => void
  ) {
    configureGlobalBrowserLaunchBroker(this.getRuntimeSettings)
  }

  async *run(job: GroupScanWorkerJob, control: ScanAdapterControl): AsyncIterable<GroupScanRawRecord> {
    ...
    const worker = utilityProcess.fork(join(__dirname, 'scanner-group-worker.js'), args, {
      serviceName: `PAGE-AUTO scanner group account ${job.accountId}`
    })
    ...
    worker.on('message', (raw) => {
      ...
      if (event.type === 'terminal') {
        queue.fail(new ScanAdapterRuntimeError(event.status, event.message))
      } else {
        queue.end()
      }
      requestShutdown()
    ...
    worker.once('exit', (code) => {
      ...
      if (!terminal) {
        ...
        else queue.fail(new ScanAdapterRuntimeError('failed', `Group scanner worker đã thoát ngoài dự kiến (code ${code}).`))
      }
    })
