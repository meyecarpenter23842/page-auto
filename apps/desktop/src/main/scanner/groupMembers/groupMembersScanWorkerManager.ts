...
function isWorkerEvent(event: unknown): event is GroupMembersScanWorkerEvent {
...
class AsyncRecordQueue implements AsyncIterator<GroupMembersScanRawRecord>, AsyncIterable<GroupMembersScanRawRecord> {
  private readonly values: GroupMembersScanRawRecord[] = []
  private readonly waiters: Array<{
    resolve: (result: IteratorResult<GroupMembersScanRawRecord>) => void
    reject: (error: Error) => void
  }> = []
  private ended = false
  private failure: Error | null = null

  push(value: GroupMembersScanRawRecord): void {
  ...
  next(): Promise<IteratorResult<GroupMembersScanRawRecord>> {
  ...
  [Symbol.asyncIterator](): AsyncIterator<GroupMembersScanRawRecord> { return this }
}

export class GroupMembersScanWorkerManager {
  private readonly active = new Set<UtilityProcess>()

  constructor(
    private readonly getRuntimeSettings: () => RuntimeSettings,
    private readonly onSessionUpdate?: (accountId: number, update: GroupMembersScanWorkerSessionUpdate) => void
  ) {
    configureGlobalBrowserLaunchBroker(this.getRuntimeSettings)
  }

  async *run(job: GroupMembersScanWorkerJob, control: ScanAdapterControl): AsyncIterable<GroupMembersScanRawRecord> {
    ...
    const worker = utilityProcess.fork(join(__dirname, 'scanner-group-members-worker.js'), args, {
      serviceName: `PAGE-AUTO scanner group members account ${job.accountId}`
    })
    ...
    worker.once('exit', (code) => {
      ...
      if (!terminal) {
        ...
        else queue.fail(new ScanAdapterRuntimeError('failed', `Group Members scanner worker đã thoát ngoài dự kiến (code ${code}).`))
      }
    })
