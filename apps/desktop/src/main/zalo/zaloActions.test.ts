...
describe('Zalo Batch 3 action modules', () => {
  ...
  it('pins production actions to the live Zalo Web search/composer DOM', () => {
    ...
    expect(sendAttachment).toContain('#chat-box-bar-id [data-translate-title="STR_SEND_PHOTO"][icon="Photo_24_Line"]')
    expect(sendAttachment).toContain('#chat-box-bar-id [icon="Photo_24_Line"]')
    expect(sendAttachment).toContain('i.fa-Photo_24_Line')
    ...
    expect(sendAttachment).toContain('#chat-box-bar-id input[type="file"]')
    expect(sendAttachment).toContain('#chat-input-container-id input[type="file"]')
    expect(sendAttachment).toContain('revealedFileInput(page, image, beforeCount)')
    expect(sendAttachment).toContain("stage: 'trigger_missing'")
    expect(sendAttachment).toContain("stage: 'chooser_missing'")
