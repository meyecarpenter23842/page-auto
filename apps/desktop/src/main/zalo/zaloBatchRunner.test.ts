...
describe('Zalo structural automation contracts', () => {
  ...
  it('reuses production action runtime while Zalo posts own their binding/media UX', () => {
    ...
    expect(runner).toContain('prepared.message')
    expect(runner).toContain('Account bị loại:')
    expect(runner).not.toContain("target.message = 'Không còn Zalo profile/session sẵn sàng để xử lý SĐT này.'")
    ...
    expect(panel).toContain('libraryHasEnabledMedia')
    expect(panel).toContain('setSendAttachment(libraryHasEnabledMedia(next))')
    expect(panel).toContain('Gửi ảnh/file đã bật theo media của bài.')
    ...
    expect(panel).toContain('onAccountsChanged?.()')
    expect(runner).not.toContain(".filter((account) => account.sessionStatus === 'ready')")
    expect(panel).not.toContain("selectedAccounts.filter((account) => account.sessionStatus === 'ready')")
    expect(panel).not.toContain("accounts.filter((account) => account.sessionStatus === 'ready').map((account) => account.id)")
    expect(panel).toContain('kiểm tra live khi Start')
